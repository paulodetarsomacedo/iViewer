...
function updateToolbarButtonsState() {
    ...
    const allToolbars = document.querySelectorAll('.text-toolbar');

    // Se nenhuma caixa de texto estiver ativa, reseta todas as toolbars para o padrão.
    if (!activeBox) {
        allToolbars.forEach(toolbar => {
            toolbar.querySelectorAll('.btn-format, .btn-align').forEach(button => button.classList.remove('active'));
            toolbar.querySelector('.font-family').value = 'Arial';
            toolbar.querySelector('.font-size').value = '12';
            toolbar.querySelector('.font-color').value = '#000000';
        });
        return;
    }
    ...
    // --- ✅ LÓGICA CORRIGIDA E SIMPLIFICADA PARA B, I, U ---
    // Itera por cada botão de formato e verifica seu estado individualmente.
    parentToolbar.querySelectorAll('.btn-format').forEach(button => {
        const textContent = button.textContent.toLowerCase();
        let command = null;
        if (textContent.includes('b')) command = 'bold';
        else if (textContent.includes('/')) command = 'italic';
        else if (textContent.includes('u')) command = 'underline';
        
        // Adiciona ou remove a classe .active com base no estado do comando.
        if (command) {
            button.classList.toggle('active', document.queryCommandState(command));
        }
    });

    // --- ✅ LÓGICA CORRIGIDA E SIMPLIFICADA PARA ALINHAMENTO ---
    // Itera por cada botão de alinhamento e verifica seu estado.
    parentToolbar.querySelectorAll('.btn-align').forEach(button => {
        let command = null;
    ...
    // A lógica para fonte e tamanho permanece a mesma da correção anterior
    const selection = window.getSelection();
    if (selection.rangeCount > 0) {
        ...
        if (fontFamilySelect && element) {
            const currentFont = window.getComputedStyle(element).fontFamily.split(',')[0].trim().replace(/['"]/g, '');
            const foundOption = Array.from(fontFamilySelect.options).find(option => option.value.toLowerCase() === currentFont.toLowerCase());
            fontFamilySelect.value = foundOption ? foundOption.value : fontFamilySelect.value;
        }

        const fontSizeSelect = parentToolbar.querySelector('.font-size');
        if (fontSizeSelect) {
            const currentSizeValue = document.queryCommandValue('fontSize');
            const reversedFontSizeMap = Object.entries(fontSizeMap).reduce((acc, [key, value]) => { acc[value] = key; return acc; }, {});
            fontSizeSelect.value = reversedFontSizeMap[currentSizeValue] || '12';
        }
    }
}
// -----------------------------------------------------------------------------
...
// --- Event Listeners para Caixas de Texto Editáveis (para atualizar o estado da toolbar E salvar range) ---
editableBoxes.forEach(box => {
    box.addEventListener('focus', () => {
        updateToolbarButtonsState();
        // --- REMOVIDO: Não aplica mais fonte e tamanho padrão ao focar na caixa ---
    });
...
// -----------------------------------------------------------------------------
// Adicionar funcionalidade de redimensionamento ao achados-panel (EXISTENTE - NÃO ALTERADA)
// -----------------------------------------------------------------------------
...
if (panel && resizer) { // Garante que os elementos existam
  ...
  document.addEventListener('mousemove', function(e) {
      ...
      // Limita a largura mínima e máxima para o painel
...
// -----------------------------------------------------------------------------
// 📋 FUNCIONALIDADE: Gerar Laudo em Nova Janela - CORRIGIDO
// -----------------------------------------------------------------------------

// ✅ VERSÃO FINAL - LOGO APARECE NO PDF
document.getElementById('btnGenerateReport').addEventListener('click', () => {

    // --- PONTO CHAVE 1: COLE O TEXTO BASE64 DO SEU LOGO AQUI (se ainda não o fez) ---
    const logoBase64 = 'data:image/png;base64,';

    // --- 1. Coleta de dados (permanece igual) ---
    // ... (toda a sua lógica de coleta de dados permanece inalterada) ...
    ...
    if (document.getElementById('tab-panoramica-btn').classList.contains('active')) {
        ...
        const denteBox = document.getElementById("dente-box");
        if (denteBox) {
            if (document.getElementById("tab-maxila").classList.contains("active")) maxilaContent = denteBox.innerHTML;
            else mandibulaContent = denteBox.innerHTML;
        }
    } else {
    ...
    // --- Função de limpeza (permanece igual) ---
    ...
    // --- Montagem do HTML (permanece igual) ---
    // ... (toda a sua lógica de montagem do reportHtmlString permanece inalterada) ...
    const includeImageCheckbox = document.getElementById('include-image-checkbox');
    const includeImage = includeImageCheckbox ? includeImageCheckbox.checked : true;
    let imageHtml = '';
    if (includeImage) {
        const canvas = document.getElementById('canvas');
        const imageDataURL = canvas.toDataURL('image/png');
        imageHtml = `<img src="${imageDataURL}" alt="Radiografia" style="max-width: 90%; height: auto; display: block; margin: 20px auto; border: 1px solid #ddd; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">`;
    }
    ...
    if (activeTabName === "Intraoral") {
        const intraoralTypeSelector = document.getElementById('intraoral-type');
        if (intraoralTypeSelector) {
            const selectedType = intraoralTypeSelector.value;
            reportHtmlString += `<h2 style="text-align: center; font-weight: bold; font-size: 18px; margin-top: 20px; margin-bottom: 20px;">${selectedType.toUpperCase()}</h2>`;
        }
    }
    if (activeTabName === "Panorâmica") {
        reportHtmlString += `<h2 style="color:rgb(172, 1, 1); text-align: center; font-weight: bold; margin-top: 15px; margin-bottom: 15px;">RADIOGRAFIA PANORÂMICA DOS MAXILARES</h2>`;
        ...
        reportHtmlString += `<h3 style="color: #000; text-align: center; font-weight: bold; margin-top: 40px; margin-bottom: 30px;">LAUDO</h3>`;
    }
    ...
    sections.forEach(section => {
        ...
        if (subtitleElement) {
            ...
            const marginTop = (subtitle.toUpperCase() === 'CONCLUSÃO') ? '40px' : '20px';
            const titleColor = (subtitle.toUpperCase() === 'CONCLUSÃO') ? '#e74c3c' : 'blue';
            ...
            if (activeTabName === "Panorâmica" && subtitle.toUpperCase() === "MAXILARES E DENTES") {
                reportHtmlString += `<h3 style="color: #555; margin-top: 10px;">Maxila:</h3><div style="line-height: 0.5;">${cleanHtml(maxilaContent)}</div>`;
                reportHtmlString += `<h3 style="color: #555; margin-top: 10px;">Mandíbula:</h3><div style="line-height: 0.5;">${cleanHtml(mandibulaContent)}</div>`;
            } else {
                const contentBox = section.querySelector('.rf-box[contenteditable="true"]');
                if (contentBox) {
                    let content = cleanHtml(contentBox.innerHTML);
                    if (!content.trim().replace(/<br>/g, '')) content = '<p>Nenhum achado.</p>';
                    if (subtitle.toUpperCase() === 'CONCLUSÃO') {
                        let tempDiv = document.createElement('div');
                        tempDiv.innerHTML = content;
                        tempDiv.querySelectorAll('p').forEach(p => { p.style.fontWeight = 'bold'; });
                        content = tempDiv.innerHTML;
                    }
                    reportHtmlString += `<div style="line-height: 0.5;">${content}</div>`;
                }
            }
        }
    });
    reportHtmlString += `<div style="text-align: center; margin-top: 50px; padding-top: 20px; border-top: 1px solid #ccc;"><img src="assinatura.png" alt="Assinatura Digital" style="max-width: 250px; height: auto;"></div>`;
    reportHtmlString += `</div>`;
    

    // --- 4. Janela de Visualização com o LOGO e os botões ---
    ...
    if (reportWindow) {
        const finalHtml = `
            ...
            <style>
                ...
                #btn-copy-laudo:hover { background-color: #0056b3; }
                #btn-generate-pdf { right: 15px; background-color: #28a745; }
                #btn-generate-pdf:hover { background-color: #218838; }

                /* ▼▼▼ AQUI ESTÁ A ÚNICA MUDANÇA ▼▼▼ */
                @media print {
                    /* Esconde APENAS os botões na impressão */
                    .action-btn {
                        display: none;
                    }
                    /* O logo NÃO é escondido, então ele aparecerá no PDF */
                ...
                <script>
                    document.getElementById('btn-copy-laudo').addEventListener('click', function() {
                        ...
                        tempContainer.innerHTML = laudoHtml;
                        ...
                        setTimeout(() => {
                            button.textContent = 'Copiar para Colar';
